...
export function createProxy<T>(
    ...
    Object.keys(descriptor.properties).forEach((propertyKey) => {
        ...
        if (
            (propertyType === ProxyPropertyType.Value$ || propertyType === ProxyPropertyType.Function$) &&
        ...
        if (propertyType === ProxyPropertyType.Value$) {
            ...
            Object.defineProperty(result, getSubscriptionKey(propertyKey), {
                ...
                get: memoize(() => (...arguments_: any[]) => (next: (value?: any) => void) => {
                    const originalObservableFunction = getProperty(
                        propertyType,
                        propertyKey,
                        descriptor.channel,
                        ObservableCtor,
            ...
            Object.defineProperty(result, propertyKey, {
                enumerable: true,
                get: memoize(() =>
                    getProperty(propertyType, propertyKey, descriptor.channel, ObservableCtor, transport),
                ),
            })
        }
    })
    ...
    switch (propertyType) {
        ...
        case ProxyPropertyType.Function$:
            return (...arguments_: any[]) =>
                makeObservable(
                    { type: RequestType.ApplySubscribe, propKey: propertyKey, args: arguments_ },
                    channel,
                    ObservableCtor,
                    transport,
                )
