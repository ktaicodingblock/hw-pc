        ...
        server
            ...
            .catch((error) => {
                if (sender !== undefined) {
                    ...
                    logger?.error?.(
                        `E-0 IPC Error on ${channel} ${stringifiedRequest} ${(error as Error).message} ${
...
class ProxyServerHandler {
    ...
    private handleSubscribe(request: SubscribeRequest, sender: WebContents): void {
        ...
        if (!isObservable(obs)) {
            console.log('XXXX not observable:', { obs })
            throw new IpcProxyError(
                `Remote property [${propKey}] is not an observable: JSON:${JSON.stringify(this.target)}`,
            )
        }
    ...
    private handleApplySubscribe(request: ApplySubscribeRequest, sender: WebContents): void {
        ...
        if (!isObservable(obs)) {
            console.log('XXXX2 not observable:', { obs })
    ...
    private doSubscribe(obs: Observable<any>, subscriptionId: string, sender: WebContents): void {
        ...
        this.subscriptions[subscriptionId] = obs.subscribe({
            ...
            error: (error: Error) =>
                sender.send(subscriptionId, { type: ResponseType.Error, error: Errio.stringify(error) }),
